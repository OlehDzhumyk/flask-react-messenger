        ...
        const fetchChats = async () => {
            try {
                ...
                if (isMounted) {
                    ...
                    // Since we normalized in service, 'participants' array guarantees to exist
                    const allParticipants = chatList.flatMap(c => c.participants || []);

                    if (allParticipants.length > 0) {
                        console.log('[Sidebar] Caching users:', allParticipants.length);
                        cacheUsers(allParticipants);
                    } else {
                        console.warn('[Sidebar] No participants found to cache.');
                    }
                }
            } catch (error) {
                        ...
                        {chats.map(chat => {
                            ...
                            return (
                                <SidebarItem
                                    key={chat.id}
                                    userId={partner.id}
                                    // Use 'name' from normalized chat or partner username
