...
* **GDPR Compliance:** "Right to be forgotten" – Complete account deletion cascades to all associated data.
...
| `GET` | `/api/users` | Search users (requires query param) |
| `GET` | `/api/chats` | List all active conversations |
| `GET` | `/api/chats/<id>/messages` | Fetch history (supports `limit` & `after_id`) |
